...
public class GUIAnalysis {
  ...
  public Set<Integer> allPreferenceIds = Sets.newHashSet();
  public Set<Integer> allPreferenceHeaderIds = Sets.newHashSet();
  public Set<String> allPreferenceKeys = Sets.newHashSet();
  ...
  public void populateIDContainers() {
    ...
    for (Integer i : xmlParser.getApplicationPreferenceIdValues()) {
      allPreferenceIds.add(i);
    }
    for (Integer i : xmlParser.getApplicationPreferenceHeaderIdValues()) {
      allPreferenceHeaderIds.add(i);
    }
    for (String i : xmlParser.getApplicationPreferenceKeys()) {
      allPreferenceKeys.add(i);
    }
    ...
    Logger.verb(this.getClass().getSimpleName(), "[XML] Layout Ids: " + allLayoutIds.size()
            ...
            + ", Preference Keys: " + allPreferenceKeys.size() + ", Preference Header Ids: " + allPreferenceHeaderIds.size() + ", Preference Ids: " + allPreferenceIds.size());
  ...
  public void run() {
    ...
    flowgraph = new Flowgraph(hier, allLayoutIds, allMenuIds, allWidgetIds,
            allStringIds, allPreferenceIds, allPreferenceHeaderIds, allPreferenceKeys);
    flowgraph.build();
