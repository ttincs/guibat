...
public class Hierarchy {
  ...
  private final SootClass navigationViewClass;
  ...
  private Hierarchy() {
    ...
    navigationViewClass = scene.getSootClass("android.support.design.widget.NavigationView");
  ...
  void serviceClasses() {
    SootClass act = Scene.v().getSootClass("android.app.Service");
    if (act == null) {
      throw new Error("[HIER] Did not find Service");
    }
    for (SootClass c : appClasses) {
      if (getSupertypes(c).contains(act) && !c.isAbstract()) {
        applicationServiceClasses.add(c);
      }
    }
    for (SootClass c : Scene.v().getLibraryClasses()) {
      if (getSupertypes(c).contains(act) && !c.isAbstract()) {
        libServiceClasses.add(c);
      }
    }
    System.out.println("[HIER] Service: " + applicationServiceClasses.size()
            + ", lib service: " + libServiceClasses.size());
  }
  ...
  void viewsAndMenus() {
    ...
    SootClass preference = scene.getSootClass("android.preference.Preference");
    SootClass sharePreference = scene.getSootClass("android.content.SharedPreferences");
    if (view == null) {
    ...
    for (SootClass c : appClasses) {
      ...
      if (superClzSet.contains(preference) || superClzSet.contains(sharePreference)) {
        preferenceClasses.add(c);
      }
    }
    int numLibViews = 0;
...
//    }
  ...
  public boolean isNavigationViewClass(final SootClass c) {
    return isSubclassOf(c, navigationViewClass);
  }

  public Set<SootClass> preferenceClasses = Sets.newHashSet();

  public Set<SootClass> applicationServiceClasses = Sets.newHashSet();
  public Set<SootClass> libServiceClasses = Sets.newHashSet();
}
