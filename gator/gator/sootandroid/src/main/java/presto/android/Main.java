...
public class Main {
  ...
  public static void parseArgs(String[] args) {
    for (int i = 0; i < args.length; i++) {
      ...
      } else if ("-permissionMapFile".equals(s)) {
        Configs.permissionMapFile = args[++i];
      } else if ("-iccModelFile".equals(s)) {
        Configs.iccModelFile = args[++i];
      } else if ("-minSdkVersion".equals(s)) {
        Configs.minSdkVersion = args[++i];
      } else if ("-maxSdkVersion".equals(s)) {
        Configs.maxSdkVersion = args[++i];
      } else if ("-targetSdkVersion".equals(s)) {
        Configs.targetSdkVersion = args[++i];
      }
      else {
