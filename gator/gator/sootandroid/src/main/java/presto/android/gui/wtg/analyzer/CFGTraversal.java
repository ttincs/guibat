...
//            filter, infeasibleEdges, infeasibleCalls);
  ...
  private static int maxDepthLevel = 350;
  private boolean traverse(
    ...
    while (!workingList.isEmpty()) {
      ...
      if (filter.match(currentStmt, currentCxt, depthLevel)) {
        if (filter.isDetectThenStop()) {
      ...
      // startActivity/showDialog/etc.
        ...
        if (!findTarget) {
          for (Unit succ : currentCFG.getSuccsOf(currentStmt)) {
            ...
            String sig = currentStmt.getInvokeExpr().getMethod().getSignature();
            if (sig.contentEquals(MethodNames.executeAsyncTaskSig)) {
              SootClass asyncTaskClass = JimpleHelper.findAsyncTaskClassAssignedToField(currentCxt, currentStmt);

              if (asyncTaskClass!=null) {

                for (SootMethod sootMethod: asyncTaskClass.getMethods()) {
                  String methodName = sootMethod.getName();
                  if (methodName.contentEquals(MethodNames.initMethodName)
                          || methodName.contentEquals(MethodNames.doInBackgroundAsyncTaskMethodName)
                          || methodName.contentEquals(MethodNames.onPostExecuteAsyncTaskMethodName)
                          || methodName.contentEquals(MethodNames.onPreExecuteAsyncTaskMethodName)
                          || methodName.contentEquals(MethodNames.onProgressUpdateAsyncTaskMethodName)) {
                    if (sootMethod.hasActiveBody()) {
                      UnitGraph tgtCFG = createOrGetCFG(methodToCFG, sootMethod);
                      for (Unit entryNode : tgtCFG.getHeads()) {
                        propagate(visitedStmts, workingList, (Stmt) entryNode, sootMethod);
                      }
                      depthLevel += 1;
                    }
                  }
