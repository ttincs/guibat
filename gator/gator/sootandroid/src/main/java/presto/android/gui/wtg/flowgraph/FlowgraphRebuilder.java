...
public class FlowgraphRebuilder {
  ...
  private final List<IccLink> iccLinks = Lists.newArrayList();
  ...
  private FlowgraphRebuilder(GUIAnalysisOutput output) {
    ...
    if (!Configs.iccModelFile.isEmpty()) {
      try {
        Ic3Provider provider = new Ic3Provider(Configs.iccModelFile);
        iccLinks.addAll(provider.getIccLinks());
      } catch (Exception ex) {
        System.out.println("ICC Model: " + ex.getMessage());
      }
    }
  }
  ...
  private NOpNode createOpNode(Stmt s) {
    ...
    {
      NOpNode startService = createStartServiceOpNode(s);
  ...
  private NOpNode createStartServiceOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    if (!wtgUtil.isServiceRelated(s)) {
      return null;
    }
    Value cxtLocal = jimpleUtil.receiver(ie);
    ...
    if (paraType instanceof RefType) {
      ...
      Logger.err(getClass().getSimpleName(), "the type of " + 0 + "'th arg is not RefType for stmt: " + s);
    }
    boolean isIntent = wtgUtil.isIntentType(baseClass);
    if (!isIntent) {
      Logger.err(getClass().getSimpleName(), "the type of " + 0 + "'th arg is not Intent for stmt: " + s);
    }
    ...
    NOpNode startServiceOpNode = new NStartServiceOpNode(activityNode, intentNode, callSite);
    return startServiceOpNode;
  }
