...
import presto.android.resources.ImageResourceParser;
...
class DefaultXMLParser extends AbstractXMLParser {
  ...
  private void doIt() {
    ...
    preferenceKeyMaps = Maps.newHashMap();
    preferencesMap = Maps.newHashMap();

    styleDrawableMap = Maps.newHashMap();

    try {
      imageResourceParser = new ImageResourceParser(Configs.project, Configs.resourceLocation);
    } catch (Exception ex) {
    ...
    readLayout();
    readMenu();
    resolvenavigationDrawer();
    readPreference();

    updateActivityLabelText();
  ...
  public Set<Integer> getApplicationPreferenceIdValues() {
    if (!preferencesMap.containsKey("preference-screen")) {
      return new HashSet<Integer>();
    }

    return preferencesMap.get("preference-screen").keySet();
  }

  @Override
  public Set<Integer> getApplicationPreferenceHeaderIdValues() {
    if (!preferencesMap.containsKey("preference-header")) {
      return new HashSet<Integer>();
    }

    return preferencesMap.get("preference-header").keySet();
  }

  @Override
  public String getApplicationPreferenceName(Integer value) {
    for(HashMap<Integer, String> map : preferencesMap.values()) {
  ...
  public Set<String> getApplicationPreferenceKeys() {
    ...
    for (Set<String> preferences : preferenceKeyMaps.values()) {
      preferenceKeys.addAll(preferences);
    }
  ...
  private void readPreference() {
    if (invRGeneralIdMap.get("xml") != null) {
      readPreference(Configs.resourceLocation + "/", invRGeneralIdMap.get("xml"), id2View, false);
    }
  }

  private String getPreferenceScreenFilePath(String menuId) {
    ...
    for (String proj : projectDirs) {
      String file = findFileExistence(proj, "xml", menuId + ".xml");
  ...
  private void readPreference(String resRoot, HashMap<Integer, String> map, HashMap<Integer, AndroidView> viewMap, boolean isLayout) {
    for (Map.Entry<Integer, String> e : map.entrySet()) {
      Integer val = e.getKey();
      String name = e.getValue();
      AndroidView root = new AndroidView();
      viewMap.put(val, root);
      String file = getPreferenceScreenFilePath(name);
      if (isLayout) {
        file =  getLayoutFilePath(resRoot, name, false);
      }
      if (file == null) {
        if (Configs.verbose) {
          System.err.println("Unknown xml " + name + " for " + resRoot);
        }
        continue;
      }
      root.setOrigin(file);
      ...
      readPreference(file, root, val);
    }
  }

  private static final String KEY_ATTR = "android:key";
  private static final String FRAGMENT_ATTR = "android:fragment";
  private HashMap<String, HashMap<Integer, String>> preferencesMap;
  private HashMap<Integer, Set<String>> preferenceKeyMaps;
    ...
    try {
      ...
      if (permissionNodes != null && permissionNodes.getLength() > 0) {
        for (int i = 0; i < permissionNodes.getLength(); i++) {
          Node currentPermissionNode = permissionNodes.item(i);
          NamedNodeMap currentPermissionNodeAttribute = currentPermissionNode.getAttributes();
          if (currentPermissionNodeAttribute != null) {
            Node attributeNameNode = currentPermissionNodeAttribute.getNamedItem("android:name");

            if (attributeNameNode != null) {
              String maxSdk = "-1";
              Node attributeMaxSdkNode = currentPermissionNodeAttribute.getNamedItem("android:maxSdkVersion");
              if (attributeMaxSdkNode != null) {
                maxSdk = attributeMaxSdkNode.getTextContent();
              }
              usesPermissions.put(attributeNameNode.getTextContent(), maxSdk);
            }
          }
        }
      }
      ...
      for (int i = 0; i < nodes.getLength(); ++i) {
        try {
          ...
          if ("activity".equals(eleName)) {
            ...
            Node labelAttr = m.getNamedItem("android:label");
            if (labelAttr != null) {
              String labelAttrValue = labelAttr.getTextContent();
              activityLabels.put(cls, labelAttrValue);
            }
...
*/
  ...
  String readAndroidTextOrTitle(NamedNodeMap attrMap, String attributeName) {
    ...
    if (textNode == null) {
      textNode = attrMap.getNamedItem("android:" + attributeName);
    }
    String text = null;
    if (textNode != null) {
  ...
  private HashMap<Integer, AndroidView> menuId2View = Maps.newHashMap();
  private HashMap<Integer, AndroidView> menuHeaderId2View = Maps.newHashMap();

  private void resolvenavigationDrawer() {
    if (menuId2View.size() == 0) return;

    for (Map.Entry<Integer, AndroidView> entry : menuId2View.entrySet()) {
      AndroidView menu = id2View.get(entry.getKey());
      entry.getValue().addChildInternal(menu);
    }

    if (menuHeaderId2View.size() == 0) return;

    for (Map.Entry<Integer, AndroidView> entry : menuHeaderId2View.entrySet()) {
      AndroidView header = id2View.get(entry.getKey());
      entry.getValue().addChildInternal(header);
    }
  }

  private static ImageResourceParser imageResourceParser;
  private HashMap<String, Set<String>> styleDrawableMap;

  @Override
  public Set<String> getImageFiles(String resName) {
    Set<String> resourceNames = new HashSet<>();

    if (imageResourceParser == null)
      return resourceNames;

    if (resName != null && resName.startsWith("?")) {
      resName = resName.substring(1);

      if (this.styleDrawableMap.containsKey(resName)) {

        //System.out.println("[INFO]: Drawable in styles " + resName);
        resourceNames.addAll(this.styleDrawableMap.get(resName));

        Set<String> imagePaths = new HashSet<>();
        for (String name : resourceNames) {
          imagePaths.addAll(imageResourceParser.getImageFiles(name));
        }
    ...
    return imageResourceParser.getImageFiles(resName);
  }
  ...
  public Set<String> getImageFiles(int resId) {
    ...
    return imageResourceParser.getImageFiles(resId);
  }

  final static String SYS_ANDROID_DRAWABLE_REF = "@android:drawable/";
  final static int SYS_ANDROID_DRAWABLE_REF_LENGTH =
          SYS_ANDROID_DRAWABLE_REF.length();

  final static String ANOTHER_SYS_ANDROID_DRAWABLE_REF = "@*android:drawable/";
  final static int ANOTHER_SYS_ANDROID_DRAWABLE_REF_LENGTH =
          ANOTHER_SYS_ANDROID_DRAWABLE_REF.length();

  final static String MOSTLY_SYS_ANDROID_DRAWABLE_REF_REF = "@drawable/";
  final static int MOSTLY_SYS_ANDROID_DRAWABLE_REF_LENGTH =
          MOSTLY_SYS_ANDROID_DRAWABLE_REF_REF.length();

  String convertAndroidDrawableToString(String androidText) {
    if (androidText.isEmpty()) {
      return null;
    }
    // Is it string ref
    if (androidText.charAt(0) == '@') {
      if (androidText.startsWith(SYS_ANDROID_DRAWABLE_REF)) {
        String value = androidText.substring(SYS_ANDROID_DRAWABLE_REF_LENGTH);
        return "@android:" + value;
      }
      if (androidText.startsWith(ANOTHER_SYS_ANDROID_DRAWABLE_REF)) {
        String value = androidText.substring(ANOTHER_SYS_ANDROID_DRAWABLE_REF_LENGTH);
        return value;
      }
      if (androidText.startsWith(MOSTLY_SYS_ANDROID_DRAWABLE_REF_REF)) {
        String value = androidText.substring(MOSTLY_SYS_ANDROID_DRAWABLE_REF_LENGTH);
        return value;
      }
      // Workaround for a weird case in XBMC
      return null;
  ...
  String readAndroidImageResource(NamedNodeMap attrMap, String attributeName) {
    Node textNode = attrMap.getNamedItemNS(ANDROID_NS, attributeName);
    if (textNode == null) {
      textNode = attrMap.getNamedItem("android:" + attributeName);
    }
    ...
    if (textNode != null) {
      String refOrValue = textNode.getTextContent();
      text = convertAndroidDrawableToString(refOrValue);
      Logger.trace("XMLParser", "  * `" + refOrValue + "' -> `" + text + "'");
    }
    return text;
  }

  String readAndroidImageResource(NamedNodeMap attrMap) {
    String images = "";

    String background = readAndroidImageResource(attrMap, "background");
    if (background != null && !background.isEmpty()) {
      images = background;
    }

    String src = readAndroidImageResource(attrMap, "src");
    if (src != null && !src.isEmpty()) {
      images += PropertyManager.SEPARATOR + src;
    }

    String icon = readAndroidImageResource(attrMap, "icon");
    if (icon != null && !icon.isEmpty()) {
      images += PropertyManager.SEPARATOR + icon;
    }

    String drawableRight = readAndroidImageResource(attrMap, "drawableRight");
    if (drawableRight != null && !drawableRight.isEmpty()) {
      images += PropertyManager.SEPARATOR + drawableRight;
    }

    String drawableLeft = readAndroidImageResource(attrMap, "drawableLeft");
    if (drawableLeft != null && !drawableLeft.isEmpty()) {
      images += PropertyManager.SEPARATOR + drawableLeft;
    }

    String drawableTop = readAndroidImageResource(attrMap, "drawableTop");
    if (drawableTop != null && !drawableTop.isEmpty()) {
      images += PropertyManager.SEPARATOR + drawableTop;
    }

    String drawableBottom = readAndroidImageResource(attrMap, "drawableBottom");
    if (drawableBottom != null && !drawableBottom.isEmpty()) {
      images += PropertyManager.SEPARATOR + drawableBottom;
    }
  ...
  public void readStyles() {
    ArrayList<String> xmlFiles = Lists.newArrayList();
    String valuesDirectoryName = Configs.resourceLocation + "/values/";
    File valuesDirectory = new File(valuesDirectoryName);
    if (!valuesDirectory.exists()) {
      System.out.println(
              "[WARNING] Directory " + valuesDirectory + " does not exist!");
    ...
    for (String file : valuesDirectory.list()) {
      if ((file.equals("styles.xml")
              || (file.startsWith("styles-")
              || file.equals("themes.xml")
    ...
    for (String file : xmlFiles) {
      readStyles(file, styleDrawableMap);
    }
  }

  private void readStyles(String file, HashMap<String, Set<String>> styleMap) {
    Document doc;
    try {
      DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
      DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
    ...
    for (int i = 0; i < nodes.getLength(); i++) {
      ...
      for (int j = 0; j < itemNodes.getLength(); j++){
        ...
        if (stringValue != null && stringValue.startsWith("@drawable/")) {
          stringValue = stringValue.substring("@drawable/".length());

          if (styleMap.containsKey(stringName)) {
            Set<String> currents = styleMap.get(stringName);
            currents.add(stringValue);
            styleMap.put(stringName, currents);
          } else {
            Set<String> newValues = new HashSet<String>();
            newValues.add(stringValue);
            styleMap.put(stringName, newValues);
          }
        }
      }
    }
  ...
  private void updateActivityLabelText() {
    HashMap<String, String> map = Maps.newHashMap();

    for (Map.Entry<String, String> entry : activityLabels.entrySet()) {
      String textValue = convertAndroidTextToString(entry.getValue());
      map.put(entry.getKey(), textValue);
    }

    activityLabels.clear();
    activityLabels.putAll(map);
  }
}
