...
public interface GUIAnalysisOutput {
  ...
  boolean isOnSharedPreferenceChanged(SootMethod handler);
  boolean isOnPreferenceClick(SootMethod handler);
  boolean isOnPreferenceChange(SootMethod handler);
  HashMap<String, String> getActivityLabels();
  HashMap<String, String> getUsesPermissions();
  Set<PermissionNode> getUsedPermissions();
  Set<String> getRequestPermissionStmts();
}
