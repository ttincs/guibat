...
public class FixpointSolver {
  ...
  public Map<NOpNode, Set<NPreferenceKeyNode>> reachingPreferenceKeys;
  ...
  public void preSolveInit() {
    ...
    reachingPreferenceKeys = Maps.newHashMap();
  ...
  public void solveCore() {
    ...
    processOnSharedPreferenceChangedCallbacks();

    // propagation
    viewAndListenerPropagation();

    processFragmentTransactionCalls();
  }

  void processOnSharedPreferenceChangedCallbacks() {
    Set<NOpNode> opNodes = NOpNode.getNodes(NSetListenerOpNode.class);
    for (NOpNode opNode : opNodes) {
      if (((NSetListenerOpNode)opNode).callSite == null) continue;
      if (!((NSetListenerOpNode)opNode).callSite.getO1().containsInvokeExpr()) continue;

      InvokeExpr invokeExpr = ((NSetListenerOpNode)opNode).callSite.getO1().getInvokeExpr();
      if (invokeExpr.getMethod().getSubSignature().contentEquals(MethodNames.registerOnSharedPreferenceChangeListenerSubSig)) {
        NPreferenceNode preferenceNode = flowgraph.activityClassToPreference.get(opNode.callSite.getO2().getDeclaringClass());

        if (preferenceNode != null) {

          LinkedList<NNode> workList = new LinkedList<>();
          workList.add(preferenceNode);

          while (!workList.isEmpty()) {
            NPreferenceNode node = (NPreferenceNode)workList.remove();
            workList.addAll(node.getChildren());

            if (node.getClassType() == Scene.v().getSootClass("android.view.header")
            || node.getClassType() == Scene.v().getSootClass("android.view.preference-headers")
            || node.getClassType() == Scene.v().getSootClass("android.view.PreferenceScreen")
            || node.getClassType() == Scene.v().getSootClass("android.view.PreferenceCategory")
            || node.getClassType() == Scene.v().getSootClass("android.view.intent")) {
              continue;
            }


            Set<NNode> receiverSolutionSet = solutionReceivers.get(opNode);
            if (receiverSolutionSet == null) {
              receiverSolutionSet = Sets.newHashSet();
              solutionReceivers.put(opNode, receiverSolutionSet);
            }
            receiverSolutionSet.add(node);
          }
        }
      }
    }
  ...
  void processFragmentTransactionCalls() {
    for (Set<NWindowNode> windows : reachingWindows.values()) {
      for (NWindowNode window : windows) {
        if (!window.c.isApplicationClass() || window instanceof NDialogNode) {
          continue;
        }

        // Handle fragment by the fragment transaction call in the app code
        {
          Set<NFragmentNode> fragmentNodes = flowgraph.activityClassToFragments.get(window.c);
          if (fragmentNodes != null) {
            for (NFragmentNode fragmentNode : fragmentNodes) {
              for (Map.Entry<NNode, Set<NOpNode>> items : viewProducers.entrySet()) {
                for (NOpNode opNode : items.getValue()) {
                  if (!(opNode instanceof NInflate1OpNode)) {
                    continue;
                  }
                  SootClass sootClass = opNode.callSite.getO2().getDeclaringClass();
                  if (sootClass == fragmentNode.fragmentClass) {

                    if (fragmentNode.parentLayoutId != null) {
                      LinkedList<NNode> workList = new LinkedList<>();
                      workList.addAll(window.getChildren());

                      while (!workList.isEmpty()) {
                        NNode node = workList.remove();
                        workList.addAll(node.getChildren());
                        if (node.idNode != null && node.idNode.getIdValue().intValue() == fragmentNode.parentLayoutId.intValue()) {
                          items.getKey().addParent(node);
                          break;
                        }
                      }
                    } else {
                      items.getKey().addParent((NNode) window.getChildren().toArray()[0]);
                    }
        ...
        // Handle fragment by the XML attributes in the layout file
        {
          LinkedList<NNode> workList = new LinkedList<>();
          workList.addAll(window.getChildren());
          ...
          while (!workList.isEmpty()) {
            NNode node = workList.remove();
            if (node instanceof NInflNode) {
              if (!((NInflNode) node).c.isApplicationClass()) {
                continue;
              }
              workList.addAll(node.getChildren());

              SootClass fragment = ((NInflNode)node).fragment;
              if (fragment == null) {
                continue;
              }
...
//        }
        ...
        if (strBldNode.possibleValues.size() >= 100) return;
        ...
        if (appendOpNode.strNode instanceof NStringConstantNode) {
          String value = ((NStringConstantNode) appendOpNode.strNode).value;
          if (value.startsWith("<android.os.Environment:")) {
            strBldNode.possibleValues.add(value);
          }
        }
      }
    }
  ...
  void solutionResultsReachability() {
    ...
    solutionResultsReachability(NOpNode.getNodes(NFindPreferenceOpNode.class));
  }
  ...
  void preferenceKeyReachability() {
    for (NPreferenceKeyNode preferenceKeyNode : flowgraph.allPreferenceKeyNodes.values()) {
      for (NNode target : graphUtil.reachableNodes(preferenceKeyNode)) {
        if (!(target instanceof NOpNode)) {
          continue;
        }
        NOpNode opNode = (NOpNode) target;

        Set<NPreferenceKeyNode> layouts = reachingPreferenceKeys.get(opNode);
        if (layouts == null) {
          layouts = Sets.newHashSet();
          reachingPreferenceKeys.put(opNode, layouts);
        }
        layouts.add(preferenceKeyNode);
      }
    }
  }
  ...
  void viewAndListenerAsParameterAndReceiverReachability() {
    ...
    for (NNode n : flowgraph.allNNodes) {
      if (n instanceof NViewAllocNode || n instanceof NInflNode
        ...
        if (opNode instanceof NFindView1OpNode
                || opNode instanceof NFindPreferenceOpNode
                || opNode instanceof NFindView2OpNode
                || opNode instanceof NFindView3OpNode
...
//        vNode = flowgraph.inflNode(v.getSootClass());
      ...
      String tooltip = v.getTooltip();
      if (tooltip != null) {
        for (String t : tooltip.split(PropertyManager.SEPARATOR)) {
          vNode.addTooltipNode(flowgraph.stringConstantNode(t));
        }
      }
      ...
      if (contentDescription != null) {
        for (String t : contentDescription.split(PropertyManager.SEPARATOR)) {
          vNode.addContentDescriptionNode(flowgraph.stringConstantNode(t));
        }
      }

      String imageResource = v.getImageResource();
      if (imageResource != null) {
        for (String t : imageResource.split(PropertyManager.SEPARATOR)) {
          vNode.addImageResourceNode(flowgraph.stringConstantNode(t));
        }
      }
  ...
  NInflNode doLayoutInflate(Integer layoutId, NNode rootparent, NVarNode inflateOpNodeReciever) {
    ...
    while (!worklist.isEmpty()) {
      ...
      String fragmentClass = v.getAttr(AndroidView.Type.ANDROID_NAME);
      if (fragmentClass != null && !fragmentClass.isEmpty()) {
        vNode.fragment = Scene.v().getSootClass(fragmentClass);
      }
  ...
  void viewAndListenerPropagation() {
    while (true) {
      ...
      for (NOpNode findPreference : NOpNode.getNodes(NFindPreferenceOpNode.class)) {
        if (processFindPreference((NFindPreferenceOpNode) findPreference)) {
          changed = true;
        }
      }
...
//          Logger.verb(this.getClass().getSimpleName(), "  * lhs-item: " + lhs);
  ...
  boolean processFindPreference(NFindPreferenceOpNode node) {
    Set<NPreferenceKeyNode> preferenceKeyNodes = reachingPreferenceKeys.get(node);
    if (preferenceKeyNodes == null || preferenceKeyNodes.isEmpty()) {

      Logger.trace(this.getClass().getSimpleName(), "[WARNING] Preference key unknown at " + node);
    ...
    for (NPreferenceKeyNode keyNode : preferenceKeyNodes) {
      LinkedList<NNode> workList = new LinkedList<>();
      workList.addAll(flowgraph.activityClassToPreference.values());

      while (!workList.isEmpty()) {
        NPreferenceNode preferenceNode = (NPreferenceNode)workList.remove();
        workList.addAll(preferenceNode.getChildren());

        NPreferenceIdNode nodeId = (NPreferenceIdNode)preferenceNode.idNode;
        if (nodeId != null && nodeId.prefKey.contentEquals(keyNode.key)) {
          solution.add(preferenceNode);
        }
      }
...
//                "SystemR.id.content used for non-activity object.");
      ...
      if (n instanceof NStringConstantNode || n instanceof NStringIdNode || n instanceof NStringBuilderNode) {
        ...
        for (NNode receiver : receiverSet) {
          if (node.type.equals(NSetTextOpNode.Type.TEXT) && receiver.addTextNode(n)) {
            ...
            changed = true;
          } else if (node.type.equals(NSetTextOpNode.Type.TOOLTIP) && receiver.addTooltipNode(n)) {
            changed = true;
          } else if (node.type.equals(NSetTextOpNode.Type.CONTENT_DESCRIPTION) && receiver.addContentDescriptionNode(n)) {
            changed = true;
          }
        }
      }
  ...
  boolean processSetImageResource(NSetImageResourceOpNode node) {
    ...
    NNode imageResourceNode = node.getParameter();
    boolean resolved = false;
    boolean changed = false;
    //    GraphUtil.verbose = true;
    for (NNode n : graphUtil.backwardReachableNodes(imageResourceNode)) {
      if (n instanceof NIntConstantNode) {
        resolved = true;
        for (NNode receiver : receiverSet) {
          if(receiver.addImageResourceNode(n)){
            changed = true;
          }
        }
      }
    ...
    //    GraphUtil.verbose = false;
    if (!resolved) {
      Logger.trace(TAG, "Image unknown at " + node);
    }
    return changed;
  }
  ...
  boolean isValidFlowById(NNode solutionNode, NOpNode call) {
    try {
      Body body = call.callSite.getO2().getActiveBody();
      UnitGraph g = new BriefUnitGraph(body);
      SmartLocalDefs ld = new SmartLocalDefs(g, new SimpleLiveLocals(g));
      Value local = call.callSite.getO1().getInvokeExpr().getUseBoxes().get(1).getValue();
      List<Unit> defUnits = ld.getDefsOfAt((Local)local, call.callSite.getO1());
      Integer viewId = null;
      String key = null;

      if (defUnits != null && defUnits.size() > 0) {
        Unit defUnit = defUnits.get(0);

        if (defUnit instanceof JAssignStmt) {
          Value leftOp = ((JAssignStmt) defUnit).getLeftOp();
          Value rightOp = ((JAssignStmt) defUnit).getRightOp();

          if (rightOp instanceof JInstanceFieldRef) {
            SootField field = ((JInstanceFieldRef)rightOp).getField();

            LinkedList<Unit> traces = Lists.newLinkedList();
            Iterator<Unit> units = body.getUnits().iterator();

            while (units.hasNext()) {
              Unit currentUnit = units.next();
              traces.addFirst(currentUnit);
              if (currentUnit == defUnit) {
                traces.remove(0);
                Value holdLocal = null;
                while (!traces.isEmpty()) {
                  Unit u = traces.remove(0);
                  if (u instanceof JAssignStmt) {
                    Value lop = ((JAssignStmt) u).getLeftOp();
                    Value rop = ((JAssignStmt) u).getRightOp();

                    if (lop instanceof JInstanceFieldRef && holdLocal == null) {
                      if (((JInstanceFieldRef) lop).getField() == field) {
                        holdLocal = rop;
                      }
                    } else if (holdLocal == lop) {
                      if (rop instanceof JCastExpr) {
                        holdLocal = ((JCastExpr) rop).getOp();
                      } if (rop instanceof JVirtualInvokeExpr) {
                        SootMethod method = ((JVirtualInvokeExpr) rop).getMethod();
                        if (method.getSubSignature().contentEquals(MethodNames.findViewByIdSubSig)) {
                          viewId = ((IntConstant)((JVirtualInvokeExpr) rop).getArg(0)).value;
                          break;
                        } else if (method.getSubSignature().contentEquals(MethodNames.findPreferenceSubSig)) {
                          key = ((StringConstant)((JVirtualInvokeExpr) rightOp).getArg(0)).value;
                          break;
                        }
                      }
                    }
            ...
            Local l = (Local)((JCastExpr)rightOp).getOp();
            List<Unit> us = ld.getDefsOfAt(l, defUnit);
            ...
            if (method.getSubSignature().contentEquals(MethodNames.findViewByIdSubSig)) {
              viewId = ((IntConstant)((JVirtualInvokeExpr) rightOp).getArg(0)).value;
            } else if (method.getSubSignature().contentEquals(MethodNames.findPreferenceSubSig)) {
              key = ((StringConstant)((JVirtualInvokeExpr) rightOp).getArg(0)).value;
            }
          }
      ...
      if (viewId != null) {
        return solutionNode.idNode.getIdValue().intValue() == viewId;
      }

      if (key != null && solutionNode instanceof NPreferenceNode) {
        return ((NPreferenceIdNode)solutionNode.idNode).prefKey.contentEquals(key);
      }
    } catch (Exception ex) {
      return true;
    }
  ...
  boolean isValidFlowByType(NNode solutionNode, NOpNode call, VarType type) {
    ...
    } else if (solutionNode instanceof NPreferenceNode) {
      //sourceType = Scene.v().getSootClass("android.preference.Preference");
      sourceType = ((NPreferenceNode) solutionNode).c;
      if (sourceType == null) {
        Logger.verb("ERROR", "in NPreferenceNode branch, classtype is null");
      }
    } else {
