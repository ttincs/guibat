...
import presto.android.permission.PermissionMap;
import presto.android.permission.PermissionNode;
import presto.android.util.JimpleHelper;
...
public class Flowgraph implements MethodNames {
  ...
  public Map<Integer, NPreferenceIdNode> allNPreferenceIdNodeNodes = Maps.newHashMap();
  public Map<Integer, NPreferenceIdNode> allNPreferenceHeaderIdNodeNodes = Maps.newHashMap();
  public Map<String, NPreferenceKeyNode> allPreferenceKeyNodes = Maps.newHashMap();
  public Map<SootClass, NServiceNode> allNServiceNodes = Maps.newHashMap();
  ...
  public Set<Integer> allPreferenceIds;
  public Set<Integer> allPreferenceHeaderIds;
  public Set<String> allPreferenceKeys;
  ...
  public Flowgraph(Hierarchy hier, Set<Integer> allLayoutIds,
                   ...
                   Set<Integer> allStringIds, Set<Integer> allPreferenceIds, Set<Integer> allPreferenceHeaderIds, Set<String> allPreferenceKeys) {
    this.hier = hier;
    this.allLayoutIds = allLayoutIds;
    this.allMenuIds = allMenuIds;
    this.allWidgetIds = allWidgetIds;
    this.allStringIds = allStringIds;
    this.allPreferenceIds = allPreferenceIds;
    this.allPreferenceHeaderIds = allPreferenceHeaderIds;
    this.allPreferenceKeys = allPreferenceKeys;
  ...
  public void buildIdNodes() {
    ...
    for (Integer i : allPreferenceIds) {
      preferenceIdNode(i);
    }
    for (Integer i : allPreferenceHeaderIds) {
      preferenceHeaderIdNode(i);
    }
  }

  public void buildPreferenceKeyNodes() {
    for (String i : allPreferenceKeys) {
      preferenceKeyNode(i);
    }
  }
  ...
  void processPreferenceToActivity() {
    for (SootClass c : hier.frameworkManaged.keySet()) {
      if (!hier.applicationActivityClasses.contains(c)) continue;

      processHeaderPreferenceToActivity(c);
      processAddPreferencesFromResourceToActivity(c, c);
      processFragmentTransactionPreferencesToActivity(c);
    }
  }

  void processFragmentTransactionPreferencesToActivity(SootClass activityClass) {
    Set<NOpNode> opNodes = NOpNode.getNodes(NFragmentTransactionOpNode.class);
    if (opNodes == null) return;

    for (NOpNode opNode : opNodes) {
      if (opNode.callSite.getO2().getDeclaringClass() != activityClass) continue;
      SootClass fragmentClass = JimpleHelper.getSootClass(((NFragmentTransactionOpNode)opNode).fragment, opNode.callSite.getO1(), opNode.callSite.getO2());

      processAddPreferencesFromResourceToActivity(activityClass, fragmentClass);
    }

  }

  void processAddPreferencesFromResourceToActivity(SootClass activityClass, SootClass fragmentClass) {
    Integer prefId = JimpleHelper.getResourceId(hier, fragmentClass, onActivityCreateSubSig, addPreferencesFromResourceSubSig);
    if (prefId == null) {
      prefId = JimpleHelper.getResourceId(hier, fragmentClass, onCreatePreferencesSubSig, addPreferencesFromResourceSubSig);
    }
    if (prefId == null) {
      prefId = JimpleHelper.getResourceId(hier,fragmentClass, onActivityCreateSubSig, setPreferencesFromResourceSubSig);
      if (prefId == null) {
        prefId = JimpleHelper.getResourceId(hier,fragmentClass, onCreatePreferencesSubSig, setPreferencesFromResourceSubSig);
      }
    }
    if (prefId == null) return;

    AndroidView view = xmlUtil.findViewById(prefId);
    if (view == null) return;

    NPreferenceNode preferenceHeaderNode = findOrCreatePreferenceNode(activityClass);
    preferenceHeaderNode.id = prefId;
    preferenceHeaderNode.ownerActivity = activityClass;
    preferenceHeaderNode.c = view.getSootClass();

    modelFromPreferenceToFragment(preferenceHeaderNode, view);
  }

  void processHeaderPreferenceToActivity(SootClass c) {
    Integer resourceId = JimpleHelper.getResourceId(hier, c, onBuildHeadersSubSig, loadHeadersFromResourceSubSig);
    if (resourceId == null) return;

    AndroidView headerView = xmlUtil.findViewById(resourceId);
    Iterator<AndroidView> iterator = headerView.getChildren();


    while (iterator.hasNext()) {
      AndroidView view = iterator.next();

      NPreferenceNode preferenceHeaderNode = findOrCreatePreferenceNode(c);
      preferenceHeaderNode.id = resourceId;
      preferenceHeaderNode.ownerActivity = c;
      preferenceHeaderNode.c = Scene.v().getSootClass("android.view.preference-headers");
      //modelFromPreferenceToFragment(preferenceHeaderNode, view);

      if (view.getAttrs()!= null) {
        String fragmentClassName = view.getAttrs().get(AndroidView.Type.ANDROID_FRAGMENT);
        if (fragmentClassName == null || fragmentClassName.isEmpty()) {
          for (Map.Entry<String, String> attr : view.getAttrs().entrySet()) {
            if (attr.getKey().contains(":fragment")) {
              fragmentClassName = attr.getValue();
              break;
            }
          }
          if (fragmentClassName == null || fragmentClassName.isEmpty()) {
        ...
        SootClass fragmentClass = Scene.v().getSootClass(fragmentClassName);

        Set<NOpNode> opNodes = NOpNode.getNodes(NPreferencesFromResourceOpNode.class);
        if (opNodes == null) continue;

        for (NOpNode opNode : opNodes) {
          if (opNode.callSite.getO2().getDeclaringClass() != fragmentClass) continue;
          Integer resId = ((NPreferencesFromResourceOpNode)opNode).resourceId;

          if (resId != null) {
            AndroidView prefView = xmlUtil.findViewById(resId);
            view.addChildInternal(prefView);

            NPreferenceNode preferenceNode = findOrCreatePreferenceNode(fragmentClass);
            preferenceNode.id = resId;
            preferenceNode.ownerActivity = fragmentClass;
            preferenceNode.c = Scene.v().getSootClass("android.view.PreferenceScreen");

            modelFromPreferenceToFragment(preferenceNode, prefView);

            preferenceNode.addParent(preferenceHeaderNode);
          }
        }
      }
  ...
  void modelFromPreferenceToFragment(NObjectNode rootNode, AndroidView rootView) {
    LinkedList<AndroidView> workList = Lists.newLinkedList();
    LinkedList<NObjectNode> nodeWorkList = Lists.newLinkedList();

    workList.add(rootView);
    nodeWorkList.add(rootNode);
    while (!workList.isEmpty()){
      AndroidView view = workList.remove();
      NObjectNode parent = nodeWorkList.remove();

      NPreferenceNode node = new NPreferenceNode();
      node.c = view.getSootClass();

      String prefKey = view.getAttrs().get(AndroidView.Type.ANDROID_KEY);
      if(prefKey!=null) {
        ...
        NPreferenceIdNode nodeId = new NPreferenceIdNode(idValue);
        nodeId.prefKey = prefKey;
        node.idNode = nodeId;
      }

//      if(view.getText() != null) {
//        node.addTextNode(stringConstantNode(view.getText()));
//      }
//      if(view.getHint() != null) {
//        node.addTextNode(stringConstantNode(view.getHint()));
//      }
//      if(view.getTooltip() != null) {
//        node.addTextNode(stringConstantNode(view.getTooltip()));
//      }
//      if(view.getContentDescription() != null) {
//        node.addTextNode(stringConstantNode(view.getContentDescription()));
//      }
//      if(view.getImageResource() != null) {
//        node.addImageResourceNode(stringConstantNode(view.getImageResource()));
//      }

      String text = view.getText();
      if (text != null) {
        for (String t : text.split(PropertyManager.SEPARATOR)) {
          node.addTextNode(stringConstantNode(t));
        }
      }

      String hint = view.getHint();
      if (hint != null) {
        for (String t : hint.split(PropertyManager.SEPARATOR)) {
          node.addHintNode(stringConstantNode(t));
        }
      }

      String tooltip = view.getTooltip();
      if (tooltip != null) {
        for (String t : tooltip.split(PropertyManager.SEPARATOR)) {
          node.addTooltipNode(stringConstantNode(t));
        }
      }

      String contentDescription = view.getContentDescription();
      if (contentDescription != null) {
        for (String t : contentDescription.split(PropertyManager.SEPARATOR)) {
          node.addContentDescriptionNode(stringConstantNode(t));
        }
      }

      String imageResource = view.getImageResource();
      if (imageResource != null) {
        for (String t : imageResource.split(PropertyManager.SEPARATOR)) {
          node.addImageResourceNode(stringConstantNode(t));
        }
      }

      Iterator<AndroidView> childViews = view.getChildren();
      while (childViews.hasNext()) {
        workList.add(childViews.next());
  ...
  public Map<SootClass, NPreferenceNode> activityClassToPreference = Maps.newHashMap();
  NPreferenceNode findOrCreatePreferenceNode(SootClass activityClass) {
    NPreferenceNode preferenceNode = activityClassToPreference.get(activityClass);
    if (preferenceNode == null) {
      preferenceNode = new NPreferenceNode();
      preferenceNode.ownerActivity = activityClass;
      activityClassToPreference.put(activityClass, preferenceNode);
      allNNodes.add(preferenceNode);
    }
    ...
    for (SootClass c : hier.appClasses) {
      ...
      for (Iterator<SootMethod> iter = Lists.newArrayList(c.getMethods()).iterator();
        ...
        while (stmts.hasNext()) {
          ...
          System.out.println(String.format("%s:%s",currentMethod.getSignature(),currentStmt));
          ...
          if (currentStmt.containsInvokeExpr()) {
            ...
            NOpNode opNode = null;
            try {
              opNode = createOpNode(currentStmt, currentMethod);
            } catch (Exception e) {
            ...
            recordInterestingCalls(currentStmt, currentMethod);
  ...
  public void build() {
    buildIdNodes();
    buildPreferenceKeyNodes();
    processFrameworkManagedCallbacks();
    processApplicationClasses();
    processServiceClasses();


    processFragmentTransactionClasses();
    processPreferenceToActivity();
  ...
  public void processServiceClasses() {
    for (SootClass c : hier.applicationServiceClasses) {
      serviceNode(c);
    }
  }

  public void processFragmentTransactionClasses() {
    Set<NOpNode> fragmentTransactionOpNodes = NOpNode.getNodes(NFragmentTransactionOpNode.class);

    for(NOpNode n : fragmentTransactionOpNodes) {
      NFragmentTransactionOpNode opNode = (NFragmentTransactionOpNode)n;
      SootClass fragmentClass = JimpleHelper.getSootClass(opNode.fragment, opNode.callSite.getO1(), opNode.callSite.getO2());

      if (fragmentClass != null) {
        SootClass activityClass = opNode.callSite.getO2().getDeclaringClass();
        Set<NFragmentNode> fragmentNodes = findOrCreateFragmentNode(activityClass);

        NFragmentNode node = new NFragmentNode();
        node.fragmentClass = fragmentClass;
        node.ownerActivity = activityClass;
        if (opNode.container != null && opNode.container instanceof IntConstant) {
          node.parentLayoutId = ((IntConstant) opNode.container).value;
        }
  ...
  public Map<SootClass, Set<NFragmentNode>> activityClassToFragments = Maps.newHashMap();
  Set<NFragmentNode> findOrCreateFragmentNode(SootClass activityClass) {
    Set<NFragmentNode> fragmentNodes = activityClassToFragments.get(activityClass);
    if (fragmentNodes == null) {
      fragmentNodes = new HashSet<NFragmentNode>();
      activityClassToFragments.put(activityClass, fragmentNodes);
    }
  ...
  // Op Nodes
  public NOpNode createOpNode(Stmt s, SootMethod currentMethod) {
    ...
    {
      NOpNode opNode = createStringBuilderAppendOpNode(s, currentMethod);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createFragmentTransactionOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createPreferencesFromResourceOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createFindPreferenceNode(s);
      if (opNode != null) {
        return opNode;
      }
    }

    // view.setBackgroundResource(resId) or imageView.setImageResource(resId)
    {
      NOpNode opNode = createViewSetImageResourceOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createToastMakeTextOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createSnackbarMakeTextOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
    ...
    {
      NOpNode opNode = createSnackbarSetActionOpNode(s);
      if (opNode != null) {
        return opNode;
      }
    }
  ...
  public NOpNode createSnackbarSetActionOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createSnackbarSetActionOpNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }
    String subsig = callee.getSignature();
    if (!subsig.contentEquals(snackbarSetActionSig1) && !subsig.contentEquals(snackbarSetActionSig2)) {
      return null;
    }

    SootMethod caller = jimpleUtil.lookup(s);

    if (ie.getArg(1) instanceof NullConstant) {
      return null;
    }

    Local listenerLocal = (Local)ie.getArg(1);
    NNode textNode = simpleNode(ie.getArg(0));
    NVarNode listenerNode = varNode(listenerLocal);

    return new NSnackbarSetActionOpNode(textNode, listenerNode, new Pair<>(s, caller), false);
  }

  public NOpNode createSnackbarMakeTextOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createSnackbarMakeTextOpNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }
    String subsig = callee.getSignature();
    Integer arg = null;
    if (subsig.equals(snackbarMakeSig1) || subsig.equals(snackbarMakeSig2)) {
      arg = 1;
    } else if (subsig.equals(snackbarSetTextSig1) || subsig.equals(snackbarSetTextSig2)) {
      arg = 0;
    }
    if (arg == null) return null;

    SootMethod caller = jimpleUtil.lookup(s);
    NNode textNode = simpleNode(ie.getArg(arg));

    return new NSnackbarMakeOpNode(textNode, new Pair<>(s, caller), false);
  }

  public NOpNode createToastMakeTextOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createToastMakeTextOpNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }
    String subsig = callee.getSignature();
    Integer arg = null;
    if (subsig.equals(toastMakeTextSig1) || subsig.equals(toastMakeTextSig2)) {
      arg = 1;
    } else if (subsig.equals(toastSetTextSig1) || subsig.equals(toastSetTextSig2)) {
      arg = 0;
    }
    if (arg == null) return null;
    ...
    return new NToastMakeTextOpNode(textNode, new Pair<>(s, caller), false);
  }

  public NOpNode createFindPreferenceNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createFindPreferenceNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }
    String subsig = callee.getSubSignature();

    if (!subsig.equals(findPreferenceSubSig)) {
      return null;
    }
    Local receiver = jimpleUtil.receiver(ie);
    if (receiver == null) {
      Logger.verb("createFindPreferenceNode",
    ...
    Value preferenceKeyVal = ie.getArg(0);
    NNode preferenceKeyNode = simpleNode(preferenceKeyVal);
    if (preferenceKeyNode == null) {
      Logger.verb(this.getClass().getSimpleName(), "[WARNING] Null preference key for " + s + " @ " + caller);
      return NOpNode.NullOpNode;
    }
    NOpNode opNode = new NFindPreferenceOpNode(preferenceKeyNode, receiverNode, lhsNode,
            new Pair<>(s, caller), false);

    if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
      Logger.verb(this.getClass().getSimpleName(), "[FindView2] id: " + preferenceKeyNode + ", act: " +
              receiverNode + ", lhs: " + lhsNode + " @ " + opNode.shortDescription());
  ...
  public NOpNode createPreferencesFromResourceOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createPreferencesFromResourceOpNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }

    String sig = callee.getSubSignature();
    Integer preferenceIndex = null;
    if (sig.contentEquals(addPreferencesFromResourceSubSig)
            || sig.contentEquals(setPreferencesFromResourceSubSig)) {
      preferenceIndex = 0;
    }
    if (preferenceIndex != null) {
      Value preference = ie.getArg(preferenceIndex);
      Integer resourceId = null;
      if (preference instanceof IntConstant) {
        resourceId = ((IntConstant) preference).value;
      }

      NPreferencesFromResourceOpNode opNode = new NPreferencesFromResourceOpNode(new Pair<>(s, jimpleUtil.lookup(s)), preference, resourceId, false);
      return opNode;
    }
  ...
  public NOpNode createFragmentTransactionOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = null;
    try {
      callee = ie.getMethod();
    } catch (Exception e) {
      Logger.trace("createFragmentTransactionOpNode", "unresolved InvokeExpr " + ie.toString());
      return null;
    }
    String sig = callee.getSubSignature();
    Integer fragmentIndex = null;
    Integer containerIndex = null;
    if (sig.contentEquals(MethodNames.add01AppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.add01FragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.attachAppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.attachFragmentTransactionSubSig)) {
      fragmentIndex = 0;
    } else if (sig.contentEquals(MethodNames.add02AppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.add02FragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.add03AppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.add03FragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.replace01AppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.replace01FragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.replace02AppSupportFragmentTransactionSubSig)
            || sig.contentEquals(MethodNames.replace02FragmentTransactionSubSig)) {
      fragmentIndex = 1;
      containerIndex = 0;
    }
    ...
    if (containerIndex != null) {
      container = ie.getArg(containerIndex);
    }

    Value fragment = ie.getArg(fragmentIndex);
    NFragmentTransactionOpNode opNode = new NFragmentTransactionOpNode(new Pair<>(s, jimpleUtil.lookup(s)), fragment, container,false);
    return opNode;
  }
  ...
  public NOpNode createSetListenerOpNode(Stmt s) {
    ...
    if (!hier.viewClasses.contains(viewType)
            ...
            && !hier.preferenceClasses.contains(viewType)) {
      return null;
    }
...
//      Logger.verb(this.getClass().getSimpleName(), "{SetMenuItemId} " + s + " @ " + currentMethod);
  ...
  public NOpNode createStringBuilderAppendOpNode(Stmt s, SootMethod currentMethod) {
    if (!Configs.enableStringAnalysis)
      return null;
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = ie.getMethod();
    String sig = callee.getSignature();
    if (!sig.equals(stringBuilderAppendSig1) && !sig.equals(stringBuilderAppendSig2)) {
      return null;
    }
    Value arg = ie.getArg(0);
    NNode strNode = simpleNode(arg);

    // TODO: currently only consider appending a String constant
    // Now support variable Environment.getExternalStorage
    if (!(strNode instanceof NStringConstantNode) && !(arg instanceof StringConstant) && !(arg instanceof NullConstant)) {
      UnitGraph g = new ExceptionalUnitGraph(currentMethod.getActiveBody());
      SmartLocalDefs ld = new SmartLocalDefs(g, new SimpleLiveLocals(g));

      List<Unit> defUnits = ld.getDefsOfAt((Local)arg, s);
      if (defUnits != null && defUnits.size() > 0) {
        Unit u = defUnits.get(0);
        if (u instanceof JAssignStmt) {
          Value rop = ((JAssignStmt) u).getRightOp();
          if (rop instanceof JStaticInvokeExpr) {
            String invokeSig = ((JStaticInvokeExpr) rop).getMethod().getSignature();
            if (invokeSig.startsWith("<android.os.Environment:")) {
              NStringConstantNode node = new NStringConstantNode();
              node.value = invokeSig;
              strNode = node;
            } else {
              return null;
            }
  ...
  public NOpNode createTextViewSetTextOpNode(Stmt s) {
    ...
    boolean isTextViewSetTooltipText =
            // hier.isSubclassOf(callee.getDeclaringClass(), Scene.v().getSootClass("android.view.View")) &&
            subSig.equals(textViewSetTooltipTextSubSig);

    boolean isTextViewSetContentDescription =
            // hier.isSubclassOf(callee.getDeclaringClass(), Scene.v().getSootClass("android.view.View")) &&
            subSig.equals(textViewSetContentDescription);

    if (!isTextViewSetText && !isTextViewSetHint
            && !isTextViewSetTooltipText && !isTextViewSetContentDescription) {
      return null;
    }
    ...
    if (isTextViewSetTooltipText) type = NSetTextOpNode.Type.TOOLTIP;
    if (isTextViewSetContentDescription) type = NSetTextOpNode.Type.CONTENT_DESCRIPTION;
  ...
  public NOpNode createViewSetImageResourceOpNode(Stmt s) {
    InvokeExpr ie = s.getInvokeExpr();
    SootMethod callee = ie.getMethod();
    String subSig = callee.getSubSignature();

    boolean isViewSetImageResource =
            (subSig.equals(viewSetImageResourceSubSig)
                    || subSig.equals(viewSetBackgroundResourceSubSig));

    if (!isViewSetImageResource) {
      return null;
    }

    NNode imageNode = simpleNode(ie.getArg(0));
    NVarNode receiverNode = varNode(jimpleUtil.receiver(ie));
    return new NSetImageResourceOpNode(imageNode, receiverNode, new Pair<>(s, jimpleUtil.lookup(s)));
  }
...
//    Logger.verb(this.getClass().getSimpleName(), "[Flowgraph] Patching " + activity);
  ...
  public void recordInterestingCalls(Stmt s, SootMethod currentMethod) {
    ...
    if (recordPermissionProtectedAPICalls(s, currentMethod)) {
      return;
    }
    if (recordRuntimePermissionRequestAPICalls(s, currentMethod)) {
      return;
    }
  }

  public Set<String> requestPermissionStmtSet = Sets.newHashSet();
  private boolean recordRuntimePermissionRequestAPICalls(Stmt unit, SootMethod context) {
    if (JimpleHelper.isRequestPermissionSig(unit)) {
        requestPermissionStmtSet.add(unit.getInvokeExpr().getMethod().getSignature());
        return true;
    }
    return false;
  }

  private PermissionMap permissionMap = PermissionMap.v();
  public Set<PermissionNode> usedPermissionSet = Sets.newHashSet();
  private boolean recordPermissionProtectedAPICalls(Stmt unit, SootMethod context) {
    if (unit.containsInvokeExpr()) {
      String sig = unit.getInvokeExpr().getMethod().getSignature();
      if (sig.contentEquals(MethodNames.uriParseSig)) {
        sig = JimpleHelper.getContentProviderUri(unit, context);
      }

      if (permissionMap.isPermissionCalled(sig)) {
        PermissionNode permissionNode = new PermissionNode(context, unit, sig, -1);
        usedPermissionSet.add(permissionNode);
      }
    } else if (unit instanceof JAssignStmt) {
      String contentProviderField = JimpleHelper.getContentProviderFieldSig(unit, context);

      if (permissionMap.isPermissionCalled(contentProviderField)) {
        PermissionNode permissionNode = new PermissionNode(context, unit, contentProviderField, -1);
        usedPermissionSet.add(permissionNode);
      }
    }
    return false;
  ...
  void processConvertedAlertDialogSetters(NDialogNode alertDialog, Stmt s) {
    ...
    if (name.equals("setCustomTitle") || name.equals("setMessage")
      ...
      NNode textNode = simpleNode(ie.getArg(0));
      alertDialog.addTextNode(textNode);
      return;
    }
...
//    MultiMapUtil.addKeyAndHashSetElement(tabHostNewTabSpecCalls, lhs, s);
  ...
  public NServiceNode serviceNode(SootClass sootClass) {
    Preconditions.checkArgument(
            sootClass != null && !sootClass.isAbstract());
    NServiceNode serviceNode = allNServiceNodes.get(sootClass);
    if (serviceNode != null) {
      return serviceNode;
    }
    serviceNode = new NServiceNode();
    serviceNode.c = sootClass;
    allNServiceNodes.put(sootClass, serviceNode);
    allNNodes.add(serviceNode);
    return serviceNode;
  }
  ...
  public NNode simpleNode(Value jimpleValue) {
    ...
    if (jimpleValue instanceof StringConstant) {
      ...
      if (allPreferenceKeys.contains(stringValue)) {
        return preferenceKeyNode(stringValue);
      }
  ...
  */
  ...
  public NPreferenceIdNode preferenceIdNode(Integer i) {
    NPreferenceIdNode x = allNPreferenceIdNodeNodes.get(i);
    if (x != null) {
      return x;
    }
    x = new NPreferenceIdNode(i);
    allNPreferenceIdNodeNodes.put(i, x);
    allNNodes.add(x);
    return x;
  }

  public NPreferenceIdNode preferenceHeaderIdNode(Integer i) {
    NPreferenceIdNode x = allNPreferenceHeaderIdNodeNodes.get(i);
    if (x != null) {
      return x;
    }
    x = new NPreferenceIdNode(i);
    allNPreferenceHeaderIdNodeNodes.put(i, x);
    allNNodes.add(x);
    return x;
  }
  public NPreferenceKeyNode preferenceKeyNode(String i) {
    NPreferenceKeyNode x = allPreferenceKeyNodes.get(i);
    if (x != null) {
      return x;
    }
    x = new NPreferenceKeyNode(i);
    allPreferenceKeyNodes.put(i, x);
    allNNodes.add(x);
    return x;
  }
}
