...
public class IntentAnalysis {
  ...
  private Map<NAllocNode, Set<NStartServiceOpNode>> intentFlowtoStartService;
  ...
  private Multimap<NStartServiceOpNode, String> startServicetoTarget;
  ...
  private Map<SootMethod, Map<NObjectNode, Multimap<NStartServiceOpNode, String>>> preciseStartServicetoTarget;
  ...
  private Multimap<NStartServiceOpNode, String> approximateStartServicetoTarget;
  ...
  private void resolvePartialPreciseIntent(
          ...
          Map<NAllocNode, Set<NStartServiceOpNode>> intentAllocToStartService
  ) {
    // this run relies on const propagation analysis
    resolveLauncherActivityIntent();
    this.intentFlowtoSetIntentContent.putAll(intentAllocToSetIntent);

    if (intentAllocToStartActivity != null) {
      this.intentFlowtoStartActivity.putAll(intentAllocToStartActivity);
    }

    if (intentAllocToStartService != null) {
      this.intentFlowtoStartService.putAll(intentAllocToStartService);
    }
  ...
  private void recordApproximateIntentResolution() {
    ...
    this.approximateStartServicetoTarget.putAll(this.startServicetoTarget);
  }

  private void reset() {
    intentFlowtoStartActivity.clear();
    intentFlowtoStartService.clear();
    intentFlowtoSetIntentContent.clear();
    intentContent.clear();
    startActivitytoTarget.clear();
    startServicetoTarget.clear();
  }
  ...
  private void copyApproxiateIntentResolution(IntentAnalysis another) {
    ...
    this.approximateStartServicetoTarget.putAll(another.approximateStartServicetoTarget);
  }
  ...
  public Collection<String> getApproximateTargetService(NStartServiceOpNode opnode) {
    return approximateStartServicetoTarget.get(opnode);
  }
  ...
  public Collection<String> getPreciseTargetService(
          SootMethod handler, NObjectNode guiObj, NStartServiceOpNode opnode) {
    Map<NObjectNode, Multimap<NStartServiceOpNode, String>> objToResolution =
            preciseStartServicetoTarget.get(handler);
    if (objToResolution == null) {
      Collection<String> targetShortNames = this.getApproximateTargetService(opnode);
    ...
    Multimap<NStartServiceOpNode, String> startServiceToTarget = objToResolution.get(guiObj);
    // if we can not find the targets through precise intent analysis,
    // try the over-approximate approach.
    if (startServiceToTarget == null || startServiceToTarget.isEmpty()) {
      Collection<String> targetShortNames = this.getApproximateTargetService(opnode);
      if (targetShortNames == null) {
        return Sets.newHashSet();
      } else {
    ...
    return startServiceToTarget.get(opnode);
  }
  ...
  public Set<IntentAnalysisInfo> getPreciseIntentContent(SootMethod handler, NObjectNode guiObj) {
    Map<NObjectNode,Map<NAllocNode,IntentAnalysisInfo>> objToResolution =
            preciseIntentContent.get(handler);
    if (objToResolution == null) {
      return Sets.newHashSet();
    ...
    Set<IntentAnalysisInfo> actions = Sets.newHashSet();
    for (IntentAnalysisInfo info : startActivityToTarget.values()) {
      if (!info.getData(IntentField.Action).contains("android.intent.action.MAIN")) {
        actions.add(info);
      }
    }
    return actions;
  ...
  private void prepare() {
    ...
    for (Map.Entry<Expr, NAllocNode> entry : guiOutput.getFlowgraph().allNAllocNodes.entrySet()) {
      ...
      for (NNode reachedNode : reachedNodes) {
        ...
        } else if (reachedNode instanceof NStartServiceOpNode) {
          Set<NStartServiceOpNode> startServiceSet = intentFlowtoStartService
                  .get(intentAllocNode);
          if (startServiceSet == null) {
            startServiceSet = Sets.newHashSet();
            intentFlowtoStartService.put(intentAllocNode, startServiceSet);
          }
          startServiceSet.add((NStartServiceOpNode) reachedNode);
  ...
  private void postAnalysis() {
    ...
    for (NOpNode startServiceOpNode : NOpNode.getNodes(NStartServiceOpNode.class)) {
      NNode intentNode = startServiceOpNode.getParameter();
      Set<NNode> backReachedNodes = queryHelper.allVariableValues(intentNode);
      for (NNode backReachedNode : backReachedNodes) {
        if (backReachedNode instanceof NAnyValueNode) {
          startServicetoTarget.put((NStartServiceOpNode)startServiceOpNode, IntentAnalysisInfo.Any);
        }
      }
    }
  }
  ...
  private boolean rebuildPropagation() {
    ...
    for (Map.Entry<NAllocNode, IntentAnalysisInfo> allocToContent : intentContent.entrySet()) {
      ...
      if (!intentFlowtoStartActivity.containsKey(intent)
              && !intentFlowtoStartService.containsKey(intent)
              && !wtgUtil.isLauncherIntent(intentInfo)) {
        continue;
      }
      ...
      for (String tgtName : tgtActivities) {
        ...
        tgtClz = wtgUtil.getServiceSootClassByNameSig(tgtName);
        NServiceNode tgtServiceNode = guiOutput.getFlowgraph().allNServiceNodes.get(tgtClz);
        if (tgtServiceNode != null) {
          Set<NNode> forwardReachedNodes = graphUtil.reachableNodes(tgtServiceNode);
          for (NNode forwardReachedNode : forwardReachedNodes) {
            // re-build the broken propagation
            if (forwardReachedNode instanceof NGetIntentOpNode) {
              NVarNode lhsNode = ((NGetIntentOpNode) forwardReachedNode).getLhs();
  ...
  private void resolveServiceIntentTarget() {
    for (Map.Entry<NAllocNode, IntentAnalysisInfo> intentToInfo : intentContent.entrySet()) {
      NAllocNode intent = intentToInfo.getKey();
      IntentAnalysisInfo intentInfo = intentToInfo.getValue();
      Set<NStartServiceOpNode> startServices = intentFlowtoStartService.get(intent);
      // if intent never reaches any startActivityOpNode, ignore it
      ...
      if (explicitTargets.isEmpty() && Configs.implicitIntent) {
        ...
        implicitTargets.add(IntentAnalysisInfo.UnknownTargetActivity);
        // build the map: startActivitytoTarget reached by implicit intent
        for (NStartServiceOpNode startService : startServices) {
          startServicetoTarget.putAll(startService, implicitTargets);
        }
      } else if (!explicitTargets.isEmpty()) {
        // build the map: startActivitytoTarget reached by explicit intent
        for (NStartServiceOpNode startService : startServices) {
          startServicetoTarget.putAll(startService, explicitTargets);
        }
      }
    }
  }

  public void resolvePreciseStartServiceTarget(
          SootMethod handler, NObjectNode guiObj,
          Set<Stmt> newIntentStmts, Set<Stmt> setIntentStmts,
          Map<Stmt, SootMethod> startServiceStmts) {
    Map<NAllocNode, Set<NStartServiceOpNode>> intentAllocToStartService = Maps.newHashMap();
    ...
    for (Stmt newIntent : newIntentStmts) {
      ...
      Set<NNode> reachedNodes = graphUtil.reachableNodes(intentAllocNode);
      for (NNode reachedNode : reachedNodes) {
        if (reachedNode instanceof NStartServiceOpNode) {
          Stmt s = ((NStartServiceOpNode) reachedNode).callSite.getO1();
          if (!startServiceStmts.containsKey(s)) {
            // eliminate unreachable startActivity
            continue;
          }
          Set<NStartServiceOpNode> startServices = intentAllocToStartService.get(intentAllocNode);
          if (startServices == null) {
            startServices = Sets.newHashSet();
            intentAllocToStartService.put((NAllocNode) intentAllocNode,
                    startServices);
          }
          startServices.add((NStartServiceOpNode) reachedNode);
        } else if (reachedNode instanceof NSetIntentContentOpNode) {
    ...
    resolvePartialPreciseIntent(handler, guiObj, intentAllocToSetIntent,null, intentAllocToStartService);
  }
  ...
  public void resolvePreciseStartActivityTarget(
    ...
    resolvePartialPreciseIntent(handler, guiObj, intentAllocToSetIntent, intentAllocToStartActivity, null);
  }
