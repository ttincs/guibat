...
import presto.android.permission.ConstantPreferenceAnalysis;
...
public class CFGAnalyzer {
  ...
  private ConstantPreferenceAnalysis constantPreferenceAnalysis;
  ...
  public CFGAnalyzer(GUIAnalysisOutput output, FlowgraphRebuilder rebuilder) {
    ...
    this.constantPreferenceAnalysis = new ConstantPreferenceAnalysis(this.guiOutput, this.rebuilder);
  ...
  public CFGAnalyzerOutput analyzeCallbackMethod(
    ...
    if (Configs.resolveContext) {
      ...
      // transitions
      if (guiElement instanceof NPreferenceNode) {
        constantPreferenceAnalysis.doAnalysis(guiElement, handler, infeasibleEdges,
                infeasibleCalls);
      } else {
...
//              "GUI:" + guiElement + "\tinfeasibleEdges: "+ infeasibleEdges.size() + "\tinfeasibleCalls:" + infeasibleCalls.size());
  ...
  private CFGAnalyzerOutput analyzeCallbackMethod(
    ...
    final Map<Stmt, SootMethod> startServiceStmts = Maps.newHashMap();
    final Map<Stmt, SootMethod> declaredPermissionStrings = Maps.newHashMap();
    ...
    final Filter<Stmt, SootMethod> filter = new Filter<Stmt, SootMethod>() {
      @Override
      public boolean match(Stmt unit, SootMethod context, Integer depthLevel) {
        ...
        if (stmtFilter.lookforShowDialog()) {
          SootClass dialogFragment = JimpleHelper.getDialogFragmentShow(unit);
          if (dialogFragment != null) {
            targetWindows.put(new NDialogFragmentNode(dialogFragment, unit, context), new Pair<>(unit, context));
          }          
        }
        ...
        if (stmtFilter.lookforStartService()) {
          if (wtgUtil.isServiceRelated(unit)) {
            startServiceStmts.put(unit, context);

            return stmtFilter.isDetectThenStop();
          }
        }

        if (stmtFilter.lookforICC()) {
          List<IccLink> iccLinks = rebuilder.getIccLinks();
          if (iccLinks == null) return false;

          for (IccLink iccLink : iccLinks) {
            if (unit.equals(iccLink.getFromU())) {
              targetWindows.put(guiWidget, new Pair<>((Stmt) iccLink.getFromU(), iccLink.getFromSM()));
            }
          }
        }

        if (stmtFilter.lookforPermissionProtectedAPI()) {
          if (unit.containsInvokeExpr()) {
            ...
            if (permissionMap.isPermissionCalled(sig)) {
              targetWindows.put(new PermissionNode(context, unit, sig, depthLevel), new Pair<>(unit, context));
              return true;
            }

            String storageAPICalledString = JimpleHelper.getExternalStoragePermissionAPI(unit, context, rebuilder, queryHelper);
            String api = permissionMap.isContainedPermissionCalled(storageAPICalledString);
            if (api != null) {
              targetWindows.put(new PermissionNode(context, unit, api, depthLevel), new Pair<>(unit, context));
              return true;
            }
            ...
            if (permissionMap.isPermissionCalled(contentProviderField)) {
              targetWindows.put(new PermissionNode(context, unit, contentProviderField, depthLevel), new Pair<>(unit, context));
              return true;
            }
          }
        }

        if (stmtFilter.lookforRequestPermission()) {
          if (JimpleHelper.isRequestPermissionSig(unit)) {
            Set<String> permissions = JimpleHelper.getPermissionsFromRequestPermissions(unit, context, rebuilder, queryHelper, jimpleUtil);
            for (String per : permissions) {
              NStringConstantNode perName = new NStringConstantNode();
              perName.value = per;
              targetWindows.put(perName, new Pair<>(unit, context));
            }
          }
        }

        if(stmtFilter.lookforToastCall()) {
          Set<NOpNode> opNodes = NOpNode.getNodes(NToastMakeTextOpNode.class);
          for (NOpNode opNode : opNodes) {
            if (opNode.callSite.getO1() == unit) {
              NToastNode toastNode = new NToastNode();
              toastNode.addTextNode(((NToastMakeTextOpNode) opNode).textNode);
              targetWindows.put(toastNode, new Pair<>(unit, context));
            }
          }
        }

        if (stmtFilter.lookforSnackbarCall()) {
          Set<NOpNode> opNodes = NOpNode.getNodes(NSnackbarMakeOpNode.class);
          opNodes.addAll(NOpNode.getNodes(NSnackbarSetActionOpNode.class));

          for (NOpNode opNode : opNodes) {
            if (opNode.callSite.getO1() == unit) {
              NSnackbarNode toastNode = new NSnackbarNode();
              if (opNode instanceof NSnackbarMakeOpNode) {
                toastNode.addTextNode(((NSnackbarMakeOpNode) opNode).textNode);
              } else if (opNode instanceof NSnackbarSetActionOpNode) {
                NSnackbarSetActionOpNode setActionOpNode = (NSnackbarSetActionOpNode)opNode;

                toastNode.addTextNode(setActionOpNode.textNode);
                toastNode.listenerClass = JimpleHelper.getSootClass(((NVarNode)setActionOpNode.listenerNode).l,
                        setActionOpNode.callSite.getO1(), setActionOpNode.callSite.getO2());
              }
    ...
    if (stmtFilter.lookforStartService()) {
      targetWindows.putAll(addStartServiceTarget(handler, guiWidget,
              newIntentStmts, setIntentContentStmts, startServiceStmts));
    }
    if (stmtFilter.lookforShowDialog()) {
  ...
  private Multimap<NObjectNode, Pair<Stmt, SootMethod>> addStartServiceTarget(
          SootMethod handler, NObjectNode guiWidget, Set<Stmt> newIntentStmts,
          Set<Stmt> setIntentContentStmts, Map<Stmt, SootMethod> startServiceStmts) {
    Multimap<NObjectNode, Pair<Stmt, SootMethod>> targetWindows = HashMultimap.create();
    for (Stmt startServiceStmt : startServiceStmts.keySet()) {
      intentAnalysis.resolvePreciseStartServiceTarget(handler, guiWidget,
              newIntentStmts, setIntentContentStmts, startServiceStmts);
      Collection<String> targetShortNames = intentAnalysis.getPreciseTargetService(handler, guiWidget,
              (NStartServiceOpNode) NOpNode.lookupByStmt(startServiceStmt));
      ...
      for (String targetShortName : targetShortNames) {
        ...
        SootClass targetService = wtgUtil.getServiceSootClassByNameSig(targetShortName);
        if (targetService == null) {
          continue;
        }
        NServiceNode serviceNode = guiOutput.getFlowgraph().allNServiceNodes
                .get(targetService);
        if (serviceNode == null) {
          continue;
//          Logger.err(getClass().getSimpleName(), "IGNORE ACTIVITY CLASS can not find node for activity name: "
//                  + targetShortName + ", class: " + targetService);
        }
        targetWindows.put(serviceNode,
                new Pair<Stmt, SootMethod>(startServiceStmt, startServiceStmts.get(startServiceStmt)));
      }
    }
